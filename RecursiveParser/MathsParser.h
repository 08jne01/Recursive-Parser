...
public:
	...
	/*template<class T> T evaluate(std::string& s, int position, const int operatorType)

	{
		std::string leftString;
		int leftEnd = 0;
		int split = 0;
		for (int i = 1, start = 1, brackets = 0; isNumeric(leftString, 0) || start || brackets; i++)

		{
			start = 0;
			if (s[position - i] == ')') brackets++, split = 1;
			else if (s[position - i] == '(')
		...
		int rightEnd = 0;
		for (int i = 1, start = 1; isNumeric(rightString, 0) || start; i++)

		{
			start = 0;
			rightString += s[position + i];
			rightEnd = position + i;
